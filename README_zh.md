...
| 💸 **倍率优先** | 按渠道倍率从低到高选择；倍率相同时优先使用渠道优先级数值较小的渠道 |

渠道倍率和优先级可在渠道设置中配置。渠道优先级默认为 `0`，仅在倍率优先模式下倍率相同时参与排序；数值越小越优先。渠道开启自动同步后，如果上游是 Sub2API，程序会同时获取该 API Key 当前生效的倍率；获取失败时保留原倍率。渠道列表会在自动同步成功获取的倍率胶囊内显示同步标志；手动修改倍率后，该标志会清除。
